...
public class AgendaNovaTransferenciaRecorrenteAction {
	...
	public void execute(int numeroOrigem, int numeroDestino, BigDecimal valor, Calendar inicio, Calendar fim,
			int diaParaExecutar, PrintWriter resposta) {
		...
		// natural?
		TransferenciaRecorrenteAgendada transferencia = new TransferenciaRecorrenteAgendada().daConta(c1).paraConta(c2)
				.noDia(diaParaExecutar).durante(new Periodo(inicio, fim)).comValor(valor);
